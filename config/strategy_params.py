"""
策略参数配置 - 优化版 v6.0 (方案D-Stage1: 灵活化第1阶段)
基于方案B回测分析优化：渐进式放宽参数，增加交易机会

优化重点（第1阶段）：
1. 降低ADX要求：30→25（适应30m周期特性）
2. 降低BTC信号阈值：60→55（从2笔增加到5-6笔）
3. 降低ETH信号阈值：65→60（从5笔增加到7-8笔）
4. 降低SOL信号阈值：60→55（保守降低，保持盈利能力）
5. 保持量价背离过滤逻辑（方案B已实现）

预期效果：交易数从11笔增加到14-18笔，收益从+4.52%提升到+5~+7%
"""
